...
test("Windows package, visible footer and release notes share one final identity", () => {
  ...
  const notes = fs.readFileSync(path.join(root, "docs", "RELEASE_NOTES_v1.5.1.md"), "utf8");
  ...
  assert.match(index, /5E<\/strong> <strong>v1\.5\.1 · 2026\.08\.11<\/strong>/);
  assert.match(main, /\[5E v1\.5\.1\]/);
  assert.match(notes, /release-title: v1\.5\.1 — 통합 누락을 바로잡은 5E/);
  assert.match(notes, /Full Changelog.*v1\.5\.0\.\.\.v1\.5\.1/);
  assert.match(desktopGuide, /release\/5E Setup 1\.5\.1\.exe/);
