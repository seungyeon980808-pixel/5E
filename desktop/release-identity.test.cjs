...
test("Windows package, visible footer and release notes share one final identity", () => {
  ...
  const notes = fs.readFileSync(path.join(root, "docs", "RELEASE_NOTES_v1.5.8.md"), "utf8");
  ...
  assert.equal(pkg.version, "1.5.8");
  ...
  assert.match(index, /5E<\/strong> <strong>v1\.5\.8 · 2026\.08\.13<\/strong>/);
  assert.match(main, /\[5E v1\.5\.8\]/);
  assert.match(notes, /release-title: v1\.5\.8 — PDF 교과서 검색·크롭 및 AI 그래프 통합/);
  assert.match(notes, /Full Changelog.*v1\.5\.7\.\.\.v1\.5\.8/);
  assert.match(desktopGuide, /release\/5E Setup 1\.5\.8\.exe/);
