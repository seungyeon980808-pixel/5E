...
test("Windows package, visible footer and release notes share one final identity", () => {
  ...
  const notes = fs.readFileSync(path.join(root, "docs", "RELEASE_NOTES_v1.5.2.md"), "utf8");
  ...
  assert.match(index, /5E<\/strong> <strong>v1\.5\.2 · 2026\.08\.11<\/strong>/);
  assert.match(main, /\[5E v1\.5\.2\]/);
  assert.match(notes, /release-title: v1\.5\.2 — 지연 자르기 UI 복원/);
  assert.match(notes, /Full Changelog.*v1\.5\.1\.\.\.v1\.5\.2/);
  assert.match(desktopGuide, /release\/5E Setup 1\.5\.2\.exe/);
