...
test("Windows package, visible footer and release notes share one final identity", () => {
  ...
  const notes = fs.readFileSync(path.join(root, "docs", "RELEASE_NOTES_v1.5.7.md"), "utf8");
  ...
  assert.match(index, /5E<\/strong> <strong>v1\.5\.7 · 2026\.08\.13<\/strong>/);
  assert.match(main, /\[5E v1\.5\.7\]/);
  assert.match(notes, /release-title: v1\.5\.7 — PDF 교과서 검색·크롭 및 AI 그래프 통합/);
  assert.match(notes, /Full Changelog.*v1\.5\.6\.\.\.v1\.5\.7/);
  assert.match(desktopGuide, /release\/5E Setup 1\.5\.7\.exe/);
