...
test("batch conversion is raster-only and starts one job per terminal completion", async () => {
  const context = await openPanel();
  const { browser } = context;
  const { controls, desktop, imageDataUrl } = browser;
  try {
    await context.manager.attachReference({ src: imageDataUrl, name: "source-a.png" });
    await context.manager.attachReference({ src: imageDataUrl, name: "source-b.png" });
    controls.mode.complete.click();
    controls.quality.simple.click();

    controls.output.asset.click();
    controls.batch.click();
    assert.equal(desktop.sends.length, 0);
    assert.equal(controls.batchGrid.children.length, 0);
    ...
    assert.equal(desktop.sends.length, 1);
    assert.deepEqual(controls.batchGrid.children.map((card) => card.dataset.state), ["running", "queued"]);
    ...
    const secondSendReady = desktop.waitForSend(1);
    completeImageTurn(desktop, first, imageDataUrl);
    const second = await secondSendReady;
    assert.equal(second.payload.purpose, "image");
    assert.equal(desktop.sends.length, 2);
    assert.deepEqual(
      controls.batchGrid.children.map((card) => card.dataset.state).sort(),
      ["complete", "running"],
    );

    const allComplete = browser.document.waitForState(() =>
      controls.batchGrid.children.length === 2
      && controls.batchGrid.children.every((card) => card.dataset.state === "complete"));
    completeImageTurn(desktop, second, imageDataUrl);
    await allComplete;
    assert.equal(desktop.sends.length, 2);
  } finally {
    closePanel(context);
