import { AI_QUALITY_MODES, normalizeQualityMode, qualityModeRule } from "./ai-quality-mode.js?v=1.6.0-preview-labeler-0917-1111";
...
- 원본이 있으면 출력 프레임의 가로세로 비율과 객체의 프레임 내 위치·크기·간격을 유지한다. 요청하지 않은 자동 중앙 재배치, 확대·축소, 여백 추가나 구도 재구성을 하지 않는다. 명시적인 배치 변경 요청은 해당 범위에만 적용한다.
- 흑백 중심의 단정한 2D 선화로 그린다. 외곽선은 내부선보다 굵게 하고, 사실적 조명·광택·그림자·질감·과도한 3D 표현은 쓰지 않는다. 회색은 물리적 구분이 필요한 영역에만 한두 단계의 균일한 평면색으로 최소 사용하며, 원본의 여백을 유지한다.`;
