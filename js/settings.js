...
} from "./preview-backgrounds.js?v=1.3.0";
// 전체 백업(요구 3): 개인 설정·라이브러리와 함께 '현재 프로젝트(그림·페이지)'도 한 파일에
// 담기 위해 프로젝트 직렬화/복원 함수를 재사용한다(project-io는 settings를 import하지 않아
// 순환 없음).
import { serialize as serializeProject, applyLoaded, migrate as migrateProject } from "./project-io.js?v=1.3.0";
...
import { exportLibraryString, importLibraryString, hasLibraryItems } from "./personal-objects.js?v=1.3.0";
// 전체 백업은 ZIP으로(이미지를 base64→바이너리 분리). 복원은 옛 단일 JSON도 자동 감지.
import { buildBackupZip, parseBackupZip, isZip } from "./backup-zip.js?v=1.3.0";
