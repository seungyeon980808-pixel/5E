...
function validateColorChunk(type, data) {
 let valid = true;
...
function validateMetadata(metadata) {
 ...
 for (const item of metadata) {
  ...
  validateColorChunk(item.type, data);
...
export async function decodeScopedPng(input) {
 ...
 if(phase!=='done') fail('IEND is missing.'); metadata=validateMetadata(metadata);
...
export async function encodeScopedPng(image,{metadata=[]}={}) {
 const checked=validateRgbaImage(image); if(checked.data.constructor!==Uint8Array) throw new TypeError('image.data must be Uint8Array.'); if(checked.width>MAX_DIMENSION||checked.height>MAX_DIMENSION||checked.data.length>MAX_RAW_BYTES) fail('dimensions exceed limits.'); const preservedMetadata=validateMetadata(metadata);
 const raw=new Uint8Array(checked.height*(checked.width*4+1)); for(let y=0;y<checked.height;y+=1)raw.set(checked.data.subarray(y*checked.width*4,(y+1)*checked.width*4),y*(checked.width*4+1)+1); await validateIccProfile(preservedMetadata); const compressed=await transform(raw,'deflate');
