...
import { openAngleArcLabelEditor } from "../tools.js?v=1.1.0";
...
export function buildGeometrySection(ctx) {
  ...
  arcLabelEditBtn.style.cssText = "padding:4px 10px;font-size: calc(11px * var(--text-scale, 1));cursor:pointer;border:1px solid var(--border);border-radius:6px;background:var(--bg-input);color:var(--text-primary);";
  ...
  AXIS_VARIANTS.forEach(({ id, label }) => {
    ...
    btn.style.cssText =
      ...
      "background:var(--bg-input);color:#ddd;cursor:pointer;font-size: calc(12px * var(--text-scale, 1));";
