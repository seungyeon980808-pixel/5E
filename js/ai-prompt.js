...
export function buildImagePrompt({ request, mode = "diagram", revision = false }) {
  const modeRule = MODE_RULES[mode] || MODE_RULES.diagram;
  return `${EXAM_STYLE_PROMPT}\n${modeRule}\n${revision ? "수정 작업: 기존 결과의 구조·배치와 요청하지 않은 영역은 그대로 보존하고, 사용자가 지정한 부분만 변경한다." : "신규 작업: 제공된 설명과 참고 이미지의 객체 수, 실루엣, 접촉 관계와 좌우 순서를 그대로 구성한다."}\n실행 규칙: 파일·저장소·문서·웹을 검색하거나 읽지 말고 추가 질문도 하지 않는다. 제공된 요청과 첨부만 사용해 imagegen 이미지 생성 도구를 정확히 1회 호출한다. 이 턴에서 결과를 다시 생성하거나 자가 수정하지 않는다. 호출 뒤 성공하면 '이미지 생성 완료', 실패하면 '이미지 생성 실패: 짧은 이유'만 한 문장으로 반환한다.\n사용자 요청: ${String(request || "").trim()}`;
}
