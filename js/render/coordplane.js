 ...
 * 사용자가 준 참고 사진(화살촉)을 픽셀 단위로 윤곽 추출(파이썬 PIL/numpy) → 행별 잡음(흐린
 * 이미지의 1px 흔들림)을 이동평균으로 지운 뒤 → 왼쪽 경계를 22점으로 다운샘플 → L_head 기준 정규화.
 * 검증: 재래스터화해 원본과 겹쳐 IoU 96.98% / Dice 98.47%(스크립트 실측). 좌우는 항상 완전 대칭
 * (한쪽만 추적해 반대쪽은 부호만 뒤집어 그린다 — 대칭이 깨질 수 없는 구조).
 * 렌더는 centripetal Catmull-Rom(α=0.5)으로 잇는다 — 균일 Catmull-Rom은 구간별 점 간격이 크게
 * 다르면(바깥쪽 15칸 vs 안쪽 4칸) 이음매에서 곡선이 출렁였다(요구: "삐뚤삐뚤" 지적의 원인).
...
const ARROW_TRACE = [
  [0.0000, 0.0159], [0.0769, 0.0271], [0.1538, 0.0430], [0.2308, 0.0595], [0.3077, 0.0783],
  [0.3846, 0.0943], [0.4615, 0.1148], [0.5385, 0.1393], [0.6154, 0.1638], [0.6923, 0.1906],
  [0.7692, 0.2216], [0.8462, 0.2532], [0.9231, 0.2852], [1.0000, 0.3133], [0.9795, 0.2430],
