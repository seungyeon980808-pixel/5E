...
Up to five browser auth sessions are allowed by default. Set `TRIAL_MAX_SESSIONS` to a positive integer to change this admission limit on the next server start. Invalid values prevent startup. `TRIAL_MAX_RUNNING_GENERATIONS` separately configures the server-wide generation limit and defaults to five; one explicit ten-crop action starts five jobs and queues five. Each browser session has its own Codex runtime, authentication directory, and generation manager; queued jobs are admitted round-robin across sessions so one busy browser cannot starve another. A browser session can retain up to ten active or queued image jobs; the eleventh request receives HTTP 429. A repeated request in the same editor workspace remains blocked until its current job has stopped. Waiting is visible in the AI panel, and queued or running work can be cancelled. Completed and cancelled jobs return their global slot after stop acknowledgement, while recent results remain available by job ID. Existing sessions can reconnect when the browser-session admission limit is full; additional sessions receive HTTP 429.
This limits browser sessions, not verified unique people. Tabs in the same browser profile share a session. Signed-out sessions occupy a slot until idle cleanup or explicit logout; logout clears the cookie and releases the session slot. Ten per-session jobs passed synthetic HTTP and editor QA, but real ChatGPT account/provider concurrency and Render memory limits have not been capacity-tested. Increasing either limit requires checking memory and latency first.
