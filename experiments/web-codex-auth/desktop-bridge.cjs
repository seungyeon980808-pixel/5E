...
class DesktopBridge {
  ...
  async call(action, body) {
    ...
    if (action === 'interrupt') {
      if (entry && entry.job === this.session.generation.job) await this.session.generation.cancel(entry.job.jobId);
      return { ok: true };
    }
  ...
  send(body) {
    ...
    if (!this.scopes.has(body.clientScope) && this.scopes.size >= 16) throw new RequestError(429, '열린 AI 작업이 너무 많습니다. 다시 로그인해 주세요.');
    const started = this.session.generation.start({ request: body.text.slice(0, 12000) }, { text: body.text, images });
    this.scopes.set(body.clientScope, { job: this.session.generation.job, scope: body.clientScope });
    return { turnId: started.jobId, renderThreadId: started.jobId, threadId: null,
      conversationId: null, ephemeralRender: true, purpose: 'image' };
  }
  events(entry, cursor = 0) {
    if (!Number.isInteger(cursor) || cursor < 0 || cursor > 3) throw new RequestError(400, 'Invalid event cursor');
    ...
    const events = [event('item/started', { item: { id: job.jobId, type: 'imageGeneration' } })];
    ...
    if (job.state !== 'running' && !job.launchPending && (!job.turnId || job.stopped)) events.push(event('turn/completed', { turn: { id: job.jobId, status: job.state === 'cancelled' ? 'interrupted' : job.state, ...(job.error ? { error: { message: job.error } } : {}) } }));
