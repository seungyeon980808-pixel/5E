function editorPanelSource(source) {
  const replacements = [
    ...
    ['if (!panel) return;', 'if (!panel) return;\n  const taskFeedback = createTaskFeedback(panel);\n  const webOutputProcessing = panel.querySelector(\'[data-ai-output-processing]\');\n  const webOutputActions = panel.querySelector(\'.ai-output-actions\');\n  if (!webOutputProcessing || !webOutputActions) throw new Error(\'결과 처리 옵션을 표시할 위치가 없습니다.\');\n  webOutputActions.prepend(webOutputProcessing);'],
