...
async function boot(authUrl = 'https://auth.openai.com/codex/device') {
  const nodes = new Map(), listeners = {}, messages = [], openings = [];
  const node = id => {
  ...
  vm.runInNewContext(readFileSync(require.resolve('./popup-login.js'), 'utf8'), {
    ...
    screen: { availWidth: 1440, availHeight: 900, availLeft: 0 }, setInterval() {}, clearInterval() {},
    fetch: async () => ({ ok: true, json: async () => ({ authUrl, userCode: 'TEST-CODE', ticket: 'b'.repeat(64) }) }),
  });
  await new Promise(resolve => setImmediate(resolve));
  return { nodes, listeners, messages, openings, window, child, copied: () => copied };
}
...
test('code stays visible beside reusable authentication popup and both close on completion', async () => {
  ...
  assert.equal(f.openings[0][0], 'https://auth.openai.com/codex/device');
