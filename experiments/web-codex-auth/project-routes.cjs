...
function createProjectRoutes({ now = Date.now } = {}) {
  ...
  async function handle(req, reply, origin) {
    ...
    try {
      ...
      if (req.method !== 'POST' || req.headers.origin !== origin || req.headers['x-5e-request'] !== '1' || !req.headers['content-type']?.startsWith('application/json')) { reply(403, '{}'); return true; }
      let size = 0;
      const chunks = [];
      for await (const chunk of req) {
        size += chunk.length;
        if (size > MAX_PROJECT_BYTES) throw new Error('프로젝트는 32MB 이하로 저장해 주세요.');
        chunks.push(chunk);
      }
      ...
      if (url.pathname === '/api/project-launch') {
        ...
        const editor = process.env.FIVE_E_PROJECT_EDITOR_URL || `${origin}/editor/`;
        ...
        if (building) { reply(503, JSON.stringify({ error: '다른 파일을 저장하고 있습니다. 잠시 후 다시 시도해 주세요.' })); return true; }
        building = true;
        try {
          const options = { json, server: process.env.FIVE_E_PROJECT_PUBLIC_API_URL || origin };
          if (target === 'win32') {
            const pkg = await createWindowsProjectPackage(options);
            try { reply(200, await fs.readFile(pkg.bundle), 'application/octet-stream'); }
            finally { await pkg.close(); }
          } else { reply(200, await projectPackageZip(options), 'application/zip'); }
        }
        finally { building = false; }
      }
    } catch (error) {
      reply(400, JSON.stringify({ error: error.code ? '실행형 프로젝트를 저장하지 못했습니다. 작업은 그대로 유지됩니다.' : error.message }));
    }
    return true;
  }
