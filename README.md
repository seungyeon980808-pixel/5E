...
<p><strong>코드 기준 버전 v1.5.3</strong> (package.json) · 아래 릴리즈 이력은 버전별 기록입니다.</p>
<p><a href="docs/RELEASE_NOTES_v1.5.0.html"><strong>v1.5.0 릴리즈 노트</strong> — 해당 버전 변경 보기</a></p>
...
- **Windows 배포** — 데스크톱 패키징 경로와 [코드 서명 정책](docs/CODE_SIGNING_POLICY.md)을 포함합니다. 이 작업 환경에서는 Windows/Electron 설치 파일을 검증하지 않았습니다.
...
현재 웹앱과 저장소에는 원본 해시를 확인한 **기출 예제 PNG 7개**가 기본 포함됩니다. 다운로드용 데스크톱 앱은 검증된 **최근 3개 학년도 PDF 72개·288쪽**을 별도 리소스로 포함합니다. 이전 매니페스트의 3,961개 PNG 원본은 외부 백업에 보존되며, 현재 저장소에 모두 포함되어 있지는 않습니다. 실행·자료팩 설정은 [통합 허브 실행 안내](docs/INTEGRATION_HUB_RUNBOOK.md)를 참고하세요. 검색은 자료의 코드·제목·메타데이터를 사용합니다.
...
- **영역 지우기** — 지우개의 자유 올가미로 이미지·SVG 자산의 일부를 투명하게 지웁니다(되돌리기 가능)
...
1. 연결할 MCP 프로세스에서 `app_pairing`을 호출합니다. 아직 연결되지 않은 상태의 `app_status`도 같은 페어링 기록을 보여 줍니다. 이 기록은 해당 프로세스가 선택한 `127.0.0.1` 포트와 capability를 한 줄에 묶은 것입니다.
2. 5E 상단의 **MCP 배지**를 누르고, 그 페어링 기록 한 줄 전체를 그대로 붙여 넣습니다. 포트와 capability를 따로 바꾸거나 다른 프로세스의 기록과 섞지 마세요.
3. 연결 후 `app_status`로 현재 5E 창과 페이지를 확인한 뒤 그림 명령을 보냅니다.

페어링 capability는 5E 모듈 메모리에만 있고, MCP 서버 프로세스가 끝나면 유효하지 않습니다. 서버를 재시작했다면 새 페어링 기록으로 다시 연결하세요. **페어링 기록은 비밀값이므로 다른 사람에게 공유하거나 스크린샷에 포함하지 마세요.** 공개 `/health` 응답에는 이 비밀이 포함되지 않으며, `/health`는 페어링 기록을 대신하지 않습니다.
