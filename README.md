...
_위 이미지는 v1.0.0 당시 화면입니다. 현재 문항 목록은 넓은 화면 3열, 중간 폭 2열, 모바일 1열로 표시됩니다._
...
- **탭** — 전체 · 문항 · 이미지 · PDF를 한 검색 화면에서 전환합니다
- **문항 필터** — 과목 · 학년도 · 시행(수능/6월/9월)을 함께 적용합니다
- **여러 단어 검색** — 공백으로 나눈 단어가 모두 있는 결과만 표시하며 `#` 표기도 같은 검색어로 처리합니다

카드를 고른 뒤 **캔버스에 삽입**을 누르면 현재 페이지에 이미지 객체로 들어오고, 그때부터 자유롭게 배치하고 고칠 수 있습니다. 도판이 없는 문항은 PDF 원문에서 필요한 영역을 직접 잘라 사용합니다.
...
MCP 연동 코드는 기존 프로젝트와의 호환성을 위해 저장소에 유지하지만, 현재 사용자 화면에서는 MCP 진입 버튼을 표시하지 않습니다. 따라서 이전 버전의 “상단 MCP 배지를 눌러 연결” 절차는 현재 화면에 적용되지 않습니다. 일반 이미지 작업은 상단의 **이미지 변환** 작업실과 그 안의 **소스 추가** 메뉴를 사용하세요.
