...
try {
 ...
 for (const rect of [[.12,.14,.36,.4],[.56,.14,.84,.4],[.12,.52,.36,.78],[.56,.52,.84,.78]]) { await draw(rect); await page.keyboard.press('Enter'); }
 for (const width of [1440,768,375]) {
 await page.setViewportSize({width,height:900}); await page.locator('[data-unilib-crop-fit]').click();
 await page.screenshot({path:join(output,`crop-four-${width}.png`)});
 }
 await page.setViewportSize({width:1440,height:900});
 await page.locator('[data-unilib-crop-ai]').click();
 await page.locator('[data-placement="advanced"]').click();
 await page.locator('[data-reference="0"]').click(); await page.locator('[data-reference="1"]').click();
 await page.locator('[data-action="add-bench"]').click();
 await page.locator('[data-reference="2"]').click(); await page.locator('[data-reference="3"]').click();
 for (const width of [1440,768,375]) { await page.setViewportSize({width,height:900}); await page.screenshot({path:join(output,`assignment-four-${width}.png`)}); }
