...
try {
  ...
  await captureFit("space-page-fit-1000");
  await page.keyboard.press("Escape");
  await page.setViewportSize({ width: 1920, height: 1080 });
  await openExpandedPage();
  await captureFit("space-page-fit-1920");
  await page.keyboard.press("Escape");
  await page.setViewportSize({ width: 1280, height: 800 });
  await openExpandedPage();
  await draw([0.12, 0.14, 0.36, 0.4]);
  assert.equal(await page.locator("[data-unilib-crop-confirm]").isVisible(), true);
  assert.match(await page.locator("[data-unilib-crop-confirm]").innerText(), /이 영역을 추가할까요\?.*Enter 추가.*취소/su);
  await page.screenshot({ path: join(output, "space-page-inline-add-confirmation.png") });
  await page.locator("[data-unilib-crop-draft-cancel]").click();
  const cancelled = await page.evaluate(() => ({
    draftHidden: document.querySelector("[data-unilib-crop-box]").hidden,
    confirmHidden: document.querySelector("[data-unilib-crop-confirm]").hidden,
    acceptedCount: document.querySelectorAll("[data-unilib-crop-remove]").length,
  }));
  assert.deepEqual(cancelled, { draftHidden: true, confirmHidden: true, acceptedCount: 0 });
  await acceptTwo();
  const actions = await page.evaluate(() => ({
    insertEnabled: !document.querySelector("[data-unilib-crop-insert]").disabled,
    objectifyEnabled: !document.querySelector("[data-unilib-crop-objectify]").disabled,
    aiEnabled: !document.querySelector("[data-unilib-crop-ai]").disabled,
    selectionButtons: document.querySelectorAll("[data-unilib-crop-select]").length,
    acceptedBoxes: document.querySelectorAll("[data-unilib-crop-accepted-layer] [data-crop-number]").length,
    finishEnabled: !document.querySelector("[data-unilib-crop-finish]").disabled,
    addPromptHidden: document.querySelector("[data-unilib-crop-confirm]").hidden,
  }));
  assert.deepEqual(actions, {
    insertEnabled: true, objectifyEnabled: false, aiEnabled: true,
    selectionButtons: 2, acceptedBoxes: 2, finishEnabled: true, addPromptHidden: true,
  });
  await page.keyboard.press("Enter");
  ...
  await page.setViewportSize({ width: 1000, height: 760 });
  await page.locator("[data-unilib-crop-fit]").click();
  const overlayRects = await page.evaluate(() => {
    const canvas = document.querySelector("[data-unilib-crop-canvas]").getBoundingClientRect();
    return [...document.querySelectorAll("[data-unilib-crop-accepted-layer] [data-crop-number]")].map((node) => {
      const box = node.getBoundingClientRect();
      return [(box.left - canvas.left) / canvas.width, (box.top - canvas.top) / canvas.height, box.width / canvas.width, box.height / canvas.height];
    });
  });
  assert.ok(overlayRects.every((rect) => rect.every((value) => Number.isFinite(value) && value >= 0 && value <= 1)));
  assert.ok(Math.abs(overlayRects[0][0] - 0.12) < 0.01 && Math.abs(overlayRects[1][0] - 0.56) < 0.01);
  report.scenarios.push({ name: "accepted-overlays-after-refit", rectangles: overlayRects });
  await page.screenshot({ path: join(output, "space-page-two-crops-actions.png") });
  await page.locator("[data-unilib-crop-select]").first().click();
  assert.equal(await page.locator("[data-unilib-crop-objectify]").isEnabled(), true);
  await page.locator("[data-unilib-crop-objectify]").click();
  await page.waitForFunction(() => window.qaObjectified.length === 1);
  report.scenarios.push({ name: "objectify-selected-crop", calls: await page.evaluate(() => window.qaObjectified) });

  await openExpandedPage();
  await acceptTwo();
  await page.locator("[data-unilib-crop-insert]").click();
  await page.waitForFunction(() => window.qaInserted.length === 2);
  report.scenarios.push({ name: "insert-all-accepted-crops", calls: await page.evaluate(() => window.qaInserted) });

  await openExpandedPage();
  await acceptTwo();
  await page.locator("[data-unilib-crop-ai]").click();
  await page.waitForSelector(".workbench-assignment-overlay");
  await page.locator('[data-placement="advanced"]').click();
  await page.locator('[data-reference="0"]').click();
  await page.locator('[data-action="add-bench"]').click();
  await page.locator('[data-reference="1"]').click();
  await page.screenshot({ path: join(output, "space-page-two-crops-ai-assignment.png") });
  await page.locator('[data-action="continue"]').click();
  await page.waitForFunction(() => window.qaIndependentCalls.length === 1);
  const aiCall = await page.evaluate(() => {
    const call = window.qaIndependentCalls[0];
    return { count: call.references.length, groups: call.groups, placement: call.placement, sources: call.references.map((item) => item.source) };
  });
  assert.deepEqual(aiCall.groups, [[0], [1]]);
  assert.equal(aiCall.placement, "advanced");
  assert.equal(aiCall.count, 2);
  assert.ok(aiCall.sources.every(({ documentId, pageNumber, rect }) => documentId === "qa.pdf" && pageNumber === 2 && rect.length === 4));
  assert.notDeepEqual(aiCall.sources[0].rect, aiCall.sources[1].rect);
  report.scenarios.push({ name: "ai-advanced-exact-groups", call: aiCall });
  report.status = "PASS";
} catch (error) {
  report.status = "FAIL";
