...
test("shared batch UI uses durable queue and visible complete state controls", async () => {
  const html = await source("index.html");
  const panel = await source("js/ai-panel.js");
  const batchUi = await source("js/ai-batch-ui.js");
  assert.match(panel, /mountDurableBatchUi/);
  assert.match(batchUi, /createBatchQueue/);
  assert.match(html, /multiple[^>]*data-ai-batch-files/);
  assert.match(html, /webkitdirectory/);
  assert.match(html, /data-ai-batch-cancel/);
  assert.match(html, /data-ai-batch-retry/);
  assert.match(html, /data-ai-batch-output/);
  for (const label of ["대기 중", "변환 중", "완료", "실패", "취소"]) assert.match(html, new RegExp(label));
});
