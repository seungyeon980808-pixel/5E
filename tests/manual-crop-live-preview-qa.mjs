...
try {
 ...
 await page.mouse.move(...at(.1,.1));await page.mouse.down();await page.mouse.move(...at(.3,.3));await page.waitForTimeout(40);
 const before=await page.locator('[data-unilib-crop-preview]').evaluate(c=>c.toDataURL());
 await page.mouse.move(...at(.5,.5));await page.waitForTimeout(40);
 const after=await page.locator('[data-unilib-crop-preview]').evaluate(c=>c.toDataURL());assert.notEqual(before,after);
 await page.evaluate(()=>{window.draws=0;const orig=CanvasRenderingContext2D.prototype.drawImage;CanvasRenderingContext2D.prototype.drawImage=function(...args){window.draws++;return orig.apply(this,args)};const stage=document.querySelector('[data-unilib-crop-stage]');for(let i=0;i<20;i++)stage.dispatchEvent(new PointerEvent('pointermove',{pointerId:1,clientX:450+i,clientY:400+i,bubbles:true}));});
 await page.waitForTimeout(40);const draws=await page.evaluate(()=>window.draws);assert.ok(draws<=1);
 ...
 await page.locator('[data-unilib-crop-ai]').click();await page.waitForSelector('[data-placement]');
 const widths=await page.locator('.workbench-assignment-modes').evaluate(e=>({panel:e.clientWidth,buttons:[...e.children].map(b=>b.getBoundingClientRect().width)}));assert.ok(widths.buttons.every(w=>w<widths.panel/3));
 await page.screenshot({path:join(output,'compact-modes.png')});
 await writeFile(join(output,'result.json'),JSON.stringify({pass:true,pendingLive:true,acceptedLive:true,coalescedDraws:draws,handles,widths}));
