...
try {
 ...
 await page.keyboard.press('Enter');
 await page.waitForFunction(()=>document.querySelectorAll('[data-unilib-crop-remove]').length===1);
 await draw([.55,.55,.8,.8]); await page.keyboard.press('Enter');
 await page.waitForFunction(()=>document.querySelectorAll('[data-unilib-crop-remove]').length===2);
 const first=page.locator('[data-accepted-crop-id]').first(); const id=await first.getAttribute('data-accepted-crop-id');
 let b=await first.boundingBox(); await page.mouse.move(b.x+b.width/2,b.y+b.height/2); await page.mouse.down(); await page.mouse.move(b.x+b.width/2+35,b.y+b.height/2+20,{steps:4}); await page.mouse.up();
 await page.waitForFunction(()=>document.querySelector('[data-unilib-crop-box]').hidden);
 assert.equal(await first.getAttribute('data-accepted-crop-id'),id); assert.equal(await page.locator('[data-unilib-crop-remove]').count(),2);
 assert.ok((await first.boundingBox()).x>b.x+30);
 const handle=first.locator('[data-crop-handle="se"]'); b=await handle.boundingBox(); await page.mouse.move(b.x+b.width/2,b.y+b.height/2);await page.mouse.down();await page.mouse.move(b.x+60,b.y+50,{steps:4});await page.mouse.up();
 await page.waitForFunction(()=>document.querySelector('[data-unilib-crop-box]').hidden);
 ...
 await openExpandedPage(); await acceptTwo(); await page.locator('[data-unilib-crop-ai]').click();
 await page.waitForSelector('[data-placement="together"]'); await page.locator('[data-placement="together"]').click(); await page.locator('[data-action="continue"]').click();
 await writeFile(join(output,'result.json'),JSON.stringify({pass:true,moved:true,resized:true,identityPreserved:true,cancelled:true,canvas:2,ai:true}));
