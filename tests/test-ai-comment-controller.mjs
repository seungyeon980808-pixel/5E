...
function fixture(){
  ...
  for(const [key,tag] of Object.entries({comments:'div',commentsApply:'button',commentsCount:'span',commentStatus:'span',commentsPanel:'section',chatPanel:'section'})){const n=doc.createElement(tag);n.dataset['ai'+key[0].toUpperCase()+key.slice(1)]='';panel.append(n);refs[key]=n;}
...
test('initial no-image controls are disabled and tab/tool aria stays synchronized',()=>{const f=fixture();assert.equal(f.refs.commentsApply.disabled,true);assert.equal(f.refs.comments.children.length,1);assert.equal(f.refs.commentsCount.textContent,'0');f.panel.emit('click',{target:f.tools[1]});assert.equal(f.tools[1].attrs['aria-pressed'],'true');assert.equal(f.tools[0].attrs['aria-pressed'],'false');f.panel.emit('click',{target:f.tabs[1]});assert.equal(f.refs.commentsPanel.hidden,true);assert.equal(f.refs.chatPanel.hidden,false);assert.equal(f.tabs[1].attrs['aria-pressed'],'true');f.controller.destroy();});
test('inline comment edits and deletes in one row while busy work disables both',()=>{const f=fixture();const item={id:'x',kind:'generated',name:'v1',comments:[{number:1,type:'point',x:25,y:50,w:0,h:0,text:''}],nextCommentNumber:2};f.images=[item];const stage=f.doc.createElement('div'),img=f.doc.createElement('img');stage.append(img);f.panel.append(stage);f.controller.bind(item,stage,img);f.controller.render();const pin=stage.querySelector('[data-ai-comment-marker]');pin.onclick({stopPropagation(){}});let row=f.refs.comments.children[0];const editor=row.querySelector('[data-ai-inline-editor]');assert.equal(f.doc.activeElement,f.panel);editor.focus();editor.value='정확히 수정';editor.oninput();assert.equal(item.comments[0].text,'정확히 수정');assert.equal(f.refs.commentsApply.textContent,'코멘트 1개 반영하기');assert.equal(f.doc.activeElement,editor);assert.equal(f.changes,1);f.busy=true;f.controller.render();row=f.refs.comments.children[0];assert.equal(row.querySelector('[data-ai-inline-editor]').disabled,true);assert.equal(row.querySelector('[data-ai-inline-delete]').disabled,true);assert.equal(f.refs.commentsApply.disabled,true);f.busy=false;row.querySelector('[data-ai-inline-delete]').onclick();assert.equal(item.comments.length,0);f.controller.destroy();});
test('pan never adds comments; area shows live noninteractive outline and adds a small pin',()=>{const f=fixture();const item={id:'x',kind:'generated',name:'v1',comments:[],nextCommentNumber:1};f.images=[item];const stage=f.doc.createElement('div'),img=f.doc.createElement('img');stage.append(img);f.panel.append(stage);f.controller.bind(item,stage,img);stage.emit('pointerdown',{target:img,clientX:10,clientY:10});stage.emit('pointermove',{target:img,clientX:30,clientY:30});stage.emit('pointerup',{target:img,clientX:30,clientY:30});assert.equal(item.comments.length,0);f.panel.emit('click',{target:f.tools[2]});stage.emit('pointerdown',{target:img,clientX:20,clientY:20});stage.emit('pointermove',{target:img,clientX:80,clientY:60});assert.equal(stage.querySelector('[data-ai-comment-drag-preview]').tagName,'div');stage.emit('pointerup',{target:img,clientX:80,clientY:60});assert.equal(item.comments.length,1);assert.equal(stage.querySelector('[data-ai-comment-drag-preview]'),null);const markers=stage.querySelectorAll('[data-ai-comment-marker]');assert.equal(markers.filter(m=>m.tagName==='button').length,1);assert.equal(markers.find(m=>m.tagName==='div').style.pointerEvents,'none');assert.equal(markers.find(m=>m.tagName==='button').style.width,undefined);assert.equal(f.refs.comments.children[0].querySelector('[data-ai-inline-editor]').value,'');f.controller.destroy();});
...
for (const type of ['point', 'area']) test(`Delete removes selected ${type}, protects text editing and busy work`,()=>{
 ...
 f.panel.emit('keydown',{key:'Delete',target:f.refs.comments.children[0].querySelector('[data-ai-inline-editor]')});assert.equal(item.comments.length,1);
