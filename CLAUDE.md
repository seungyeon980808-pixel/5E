...
- 코드 버전 **v1.4.0** — 마지막 GitHub Release도 v1.4.0 (2026-08-01)
- **버전 범프는 릴리즈 시점에만.** 커밋을 쌓다가 묶어서 올린다.
  임의로 올리지 않는다(UI 푸터 문자열 + `?v=` 캐시버스트 497곳 + `js/main.js` 시범공개
  배너가 한 값으로 묶여 있다). 다음 번호는 **마지막 GitHub Release 기준**으로 센다.
  ⚠️ `js/svg-to-objects.js:24` 의 `v1.3.0` 은 **과거 이력 주석**이니 같이 올리지 말 것.
- 2026-07-23 **브랜치 대통합**: 살아 있던 브랜치를 전부 main에 병합·push하고 워크트리를
  11개 지웠다. 이제 **워크트리 2개**(`5E_main`=main, `branches/5E_ai_dev`=챗봇 보류)뿐이고
  `integration-hub`는 없앴다. **새 브랜치는 main에서 딴다** → `docs/BRANCH_MAP.md`

## 작업 전에 반드시

1. **어느 폴더인지 확인한다.** 기본은 `5E_main`이다(`docs/BRANCH_MAP.md`).
...
- 코드·UI·동작을 수정한 뒤에는 `preview/index.html` 하단의 버전명 옆에
  **사람이 알아볼 수 있는 수정 문구와 짧은 식별 코드**를 반드시 표시한다.
  예: `v1.6.0 Preview · 인증 연결 수정 [AUTH-0921] · 2026.09.21`
- 수정 문구는 이번 수정의 핵심을 한국어로 짧게 적고, 식별 코드는 영문 대문자와 날짜 또는
  순번을 조합해 이전 수정본과 구분되게 만든다.
- 이 표시는 릴리즈 버전 범프와 별개다. 사용자의 명시적 지시가 없으면 버전 번호는 올리지 않고
  수정 문구·식별 코드·날짜만 갱신한다.
- 작업 완료 전 로컬 브라우저에서 하단 식별 문구가 실제로 보이는지 확인한다.
- 사용자가 따로 묻지 않아도 완료 보고에 **현재 수정본의 전체 표시명과 식별 코드**를 반드시 알린다.
- 파일을 수정하지 않은 조사·검토·기록 작업에는 새 수정본 코드를 만들지 않는다.
