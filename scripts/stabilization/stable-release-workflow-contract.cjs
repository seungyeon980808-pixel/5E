...
const EXPECTED = Object.freeze({
  ...
  jobs: {
    "verify-and-build": {
      metadata: { "runs-on": "windows-latest", "timeout-minutes": 60 },
      steps: [
    ...
    "publish-draft": {
      metadata: { needs: "verify-and-build", "runs-on": "windows-latest", "timeout-minutes": 15, environment: "stable-release", permissions: { contents: "write" } },
      steps: [
        { uses: CHECKOUT, with: { ref: SHA, "fetch-depth": 0, "persist-credentials": false } },
