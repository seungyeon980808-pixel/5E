...
`npm run package:win`은 현재 검증 버전의 `release/5E Setup 1.5.0-beta.1.exe` NSIS 설치 파일을 만든다. 설치 후 5E는 기존 `index.html`과 ES 모듈을 그대로 사용하고, AI 기능만 안전한 preload IPC로 추가된다.
