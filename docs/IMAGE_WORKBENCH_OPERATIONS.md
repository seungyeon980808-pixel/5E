## 2026-09-09 물체별 분리 작업 체크포인트

- 작업 브랜치: codex/editable-image-groups. 로컬 19414 화면의 제품 코드는 별도 5E-editable-image-groups 작업트리에서 실행한다. 설치본 교체나 원래 feat/image-improvement 브랜치 병합은 하지 않았다.
- 안정화 커밋 ee8dc4af, 물체 분리 기능 커밋 e2d7ef7a. 이번 체크포인트는 기존 구현 상태 보존이다.
- 거의 흰 배경(RGB253–255)과 4/5/1 배치의 실제 생성 PNG를 10개 투명 자산으로 분리·캔버스 삽입했다. 원본 PNG는 보존하며 유지 RGBA와 전체 전경 할당을 검사한다. 추가 AI 호출0.
- 실제 화면에서 개별 이동·이동 되돌리기·새로고침 후 복구 확인. 새 npm test501/501 통과. 회귀 입력 PNG 두 장은 tests/fixtures/separated-assets에 원본 바이트로 보존해 임시 증거 폴더 없이 테스트 가능하다.
- 현재 '분리 결과 확인'에서 분리되며 임시 물체 이름이 라벨로 삽입된다. 사용자는 자동 라벨 삽입을 원하지 않는다고 지적했다. 변환 직후 자동 분리 미리보기와 기본 라벨 제거는 아직 미구현 후속 작업이다.
- 겹친 물체의 보이지 않는 부분 복원·교과서 원래 배치 복원은 아직 구현하지 않았다.
- 로컬 상세 증거: .omo/evidence/live-separated-fix/REPORT.md. 실행 로그·인증 정보·임시 자료는 커밋에서 제외한다.

## 2026-09-09 사용자 위임 실제 제품 검사
- 새 요청에 따라 실제 제품 버튼으로 AI 1회 실행. 원본 업로드→PNG 생성→다운로드·재읽기→페이지 중앙 삽입 성공. 약63.984초, 출력1949×807. AI 응답/표시/다운로드/삽입 바이트 동일.
- 이번 PNG C2PA softwareAgent는 gpt-image / version 2.0. Image 2.5 확인 아님. 요청 실행 모델은 기존 gpt-5.6-sol / medium / priority 유지.
- 미세 색 편차 존재(최대 RGB 차9), 엄밀한 무채색 아님. 후처리·자동 재생성 없음. 브라우저+실제 backend 검사이며 Electron 설치본 검사는 아님.
- 상세 근거: .omo/evidence/real-user-test-20260909/REPORT.md. 실제 호출1, 추가 호출0. 이번 제품 코드 변경·설치본 교체·커밋·푸시 없음.

## 2026-09-09 야간 안정화 변경 실제 프로젝트 반영

- 사용자의 반영 요청에 따라 야간 브랜치의 검증된7개 파일 변경만 적용. 기존 미추적 `.omo/`, `docs/USER_FEEDBACK_STYLE_SPEC.md` 보존. 백업 및 파일 지문은 `.omo/evidence/night-integration/integration.json`에 기록.
- 임시저장 flush/순서 보존, 교체 중 대상 변경 차단, 중앙 삽입, 취소 안내 정리 반영. 새 npm test475/475 및 diff-check 통과.
- 실제19382 서버가 수정된3개 제품 JS를 디스크와 동일한 바이트로 제공함을 확인. 기존 탭 새로고침·복구 후 작업2개 및 원본/결과2개 복원, 데이터URL 길이 전후 일치, AI 사용 가능 표시 확인. 사용자 변환 테스트 준비 상태이며 이번 실제AI 호출0회.
- 서버 재시작/설치본 교체/커밋/푸시 없음. 로컬 브라우저 경로 반영이며 Electron 설치본 반영으로 간주하지 않음.
