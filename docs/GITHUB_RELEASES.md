...
게시 전에 저장소 관리자가 GitHub의 `stable-release` 환경을 직접 만들고, 한 명 이상의 필수 검토자와 자체 검토 방지를 설정해야 한다. 워크플로는 job-scoped `actions: read` 권한으로 이 설정을 조회하며, 환경이나 검토 규칙이 없으면 빌드와 게시를 시작하지 않는다. 워크플로는 이 설정을 변경하지 않는다.
...
태그 푸시 후 GitHub Actions의 `Windows Stable Release` 작업은 보호 환경 승인을 거쳐 설치 파일과 체크섬이 포함된 초안 릴리스를 만든다. 초안을 공개하기 전에 최소 지원 Windows 버전, Codex 설치·로그인 요구사항, 알려진 제한, 서명 상태를 확인한다.
