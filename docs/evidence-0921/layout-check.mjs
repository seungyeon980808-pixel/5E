...
for(const width of [1440,1024,768,375]){
 ...
 let m=await measure(); assert.ok(m.h.height>=19);assert.ok(Math.abs(m.v.bottom-m.canvas.bottom)<1);if(width>=768)assert.ok(Math.abs(m.inspector.left-m.controls.left)<1);out.push({width,mode:'normal',...m});
