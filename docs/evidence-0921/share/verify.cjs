...
async function installFixture(page) {
  await page.evaluate(async () => {
    window.shareFixture = { fail: false, calls: [] };
    window.fiveEDesktop = {
      sharingRequest: async ({ method }) => {
        window.shareFixture.calls.push(method);
        if (window.shareFixture.fail) throw Error('테스트 서버 응답 실패');
        return method === 'POST'
          ? { id: 'a'.repeat(48), revokeKey: 'fixture-only', expiresAt: new Date(Date.now() + 3600000).toISOString() }
          : { ok: true };
      },
    };
...
(async () => {
  ...
  try {
    ...
    const result = {
      ...
      browser: browserName,
      checks: ['idle 2.4s blue orbit', 'activation 420ms shimmer', 'immediate result reveal', '180ms emerald static complete', 'active-share reopen', 'reduced motion', 'failure resets idle', 'revoke resets idle', 'mobile dialog bounds'],
      stateEvidence,
